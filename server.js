...
app.use(cors()); // dev: cho phép mọi origin; khi deploy set origin cụ thể
...
async function readUsersJson(){
  try {
    const t = await fs.readFile(USERS_JSON, 'utf8');
    return JSON.parse(t || '[]');
  } catch(e){
    return [];
  }
...
app.post('/register', async (req,res)=>{
  ...
  try {
    ...
    if(rows.length>0){ await conn.end(); return res.status(409).send('Tên tài khoản đã tồn tại'); }
    ...
    // append to users.json (local backup)
    const users = await readUsersJson();
    users.push({ id: r.insertId, username, display, email, password_hash: hashed, created_at: new Date().toISOString() });
